...
NumberNode.prototype.validate = function validate(value, path) {
  ...
  if (
    ...
    throw new ConfigError(
      util.format('Expect "%d" to be strict equal with "%d"', value, this.options.strictEqualTo),
      path
    );
  }

  if (
    this.options.equalTo !== null &&
    expect.isEqual(value, this.options.equalTo) === false
  ) {
    throw new ConfigError(
